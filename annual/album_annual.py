...
def run():
    for yr in range(2018, date.today().year + 1):
        ...
        sql += "WHERE YEAR(log.logdate) = " + str(yr) + " and album.albumtypeid <> 16 GROUP BY Artist, Album;"
        ...
        base_filename = "Album Chart (Annual) - {}.txt".format(yr)
        ...
        with open(os.path.join(full_dir, base_filename), 'w', encoding='utf-8') as outfile:
            ...
            for a in chart_array:
                ...
                common.add_chart_history(yr, 0, 0, albumid, rank, weightedscore, 0)
